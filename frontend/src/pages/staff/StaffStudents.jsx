...
export default function StaffStudents() {
  ...
  const handleEditDiscountSelect = (e) => {
    ...
    setEditCouponInfo({
      ...
      finalFees:
        ...
        (Number(editForm.totalFees) * selected.percentage) / 100,
  ...
  const validateEditCoupon = async () => {
    ...
    try {
      ...
      showAlert(
        'success',
        `Coupon applied! ${data.percentage}% off → Final: ₹${data.finalFees.toLocaleString('en-IN')}`,
      )
    } catch (err) {
      showAlert('error', err.response?.data?.message || 'Invalid coupon')
    } finally {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      await api.put(`/students/${selectedStudent._id}`, fd, {
        headers: { 'Content-Type': 'multipart/form-data' },
      })
